...
class Storage(object):
    ...
    def get_single_page_X_from_html(self, html: str):
        parser._reset()
        parser.feed(html)
        root = parsel.Selector(html)
        xseq = root.xpath(".//a|.//button")
        return xseq
        ...
        with io.open(info_path, encoding='utf8') as f:
            for row in csv.DictReader(f):
                ...
                if language == None:
                    ...
                    if row['Checked'] == file_type and (row['Language'] in language if isinstance(language, list) else row['Language'] == language):
                        yield row
